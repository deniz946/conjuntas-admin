...
.controller('listCtrl', function ($scope, $http, $route, API, LANG, $cookies) {
	...
	$scope.addComment = function(pack){
		swal({
			title: "Añadir una nueva actualización",
			text: "Comentario:",
			type: "input",
		...
		function(inputValue){
			...
			$http.post(API + 'pack/comment/' + pack._id, {comment: inputValue}).then(function(response){
				if(response.data.err) swal('Err', response.data.msg, 'error');
